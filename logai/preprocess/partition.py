...
class Partitioner:
    ...
    def group_counter(self, logrecord_df: pd.DataFrame):
        ...
        if group_by_timestamps is not None:
            selected_df[constants.LOG_TIMESTAMPS] = logrecord_df[constants.LOG_TIMESTAMPS]
            group_bys += [pd.Grouper(key=constants.LOG_TIMESTAMPS, freq=group_by_timestamps, offset=0, label='left')]

        grouped_df = logrecord_df.groupby(
            by=group_bys,
            as_index=False).size().rename(columns={'size': constants.LOG_COUNTS})
    ...
    def group_sliding_window(
            ...
            logline_col_name=constants.LOGLINE_NAME):
        """
        ...
        if not logline_col_name in logrecord_df.columns:
            raise ValueError("Logline column {} cannot be found in logrecord_df.columns".format(logline_col_name))
        ...
        if group_by_timestamps and group_by_category:
            ...
            grouper = [pd.Grouper(key="timestamp", freq=group_by_timestamps, base=0, label='left')] + group_by_category

        elif group_by_timestamps:
            partition_cols = [constants.LOG_TIMESTAMPS]
            grouper = [pd.Grouper(key="timestamp", freq=group_by_timestamps, base=0, label='left')]
    ...
    def _sliding_window(self, loglines: pd.Series) -> list:
        ...
        windows = loglines.rolling(window=self.config.sliding_window)
        partitioned_loglines = []
        for window in windows:
