...
class StructuredLogAnomalyDetection:
    """Class for doing anomaly detection over Structured Logs in MQ.
    ...
    def __init__(
        self, anomaly_detection_by_metric_or_freq: str = None, log_metric: str = None
    ):
        """initializing

        Args:
            anomaly_detection_by_metric_or_freq (str, optional): anomaly detection should be based on metric or frequency count. Defaults to None.
            log_metric (str, optional): Type of metric for which anomaly detection should be done. Defaults to None.

        Raises:
            Exception: if type of log_metric is not mentioned
        """
        ...
        if self.anomaly_detection_by_metric_or_freq == "freq_count":
            ...
            self.count_feature_extractor = FeatureExtractor(
                self.count_partitioning_config
            )
        ...
        if self.anomaly_detection_by_metric_or_freq == "metric":
            ...
            self.metric_feature_extractor = FeatureExtractor(
                self.metric_partitioning_config
            )

    def structured_log_anomaly_detection(
        self, incident_filepath, reference_filepath, stats_per_msgtype={}
    ):
        """workflow for anomaly detection on structured MQ logs

        Args:
            incident_filepath (str): absolute path to file containing incident period data
            reference_filepath (str): absolute path to file containing reference period data
            stats_per_msgtype (dict, optional): Contains results for each message type. This is needed if
                the anomaly detection need to use multiple reference periods. Defaults to {}.

        Returns:
            dict: stats_per_msgtype: dict updated with the anomaly detection results obtained from this method
        """
        ...
        if self.anomaly_detection_by_metric_or_freq == "freq_count":
            stats_per_msgtype = self.anomaly_detection_with_freq_count_per_msgtype(
                incident_logrecord, reference_logrecord, stats_per_msgtype
            )
            stats_per_msgtype = (
            ...
            stats_per_msgtype = self.anomaly_detection_with_metric_per_msgtype(
                incident_logrecord, reference_logrecord, stats_per_msgtype
            )
            stats_per_msgtype = self.joint_anomaly_detection_with_metric_all_msgtype(
                incident_logrecord, reference_logrecord, stats_per_msgtype
            )
        return stats_per_msgtype

    def load_data(self, filepath):
        """Method to load log data, given the filepath

        Args:
            filepath (str): path to file containing log data

        Returns:
            LogRecordObject : logrecord object created from file contents
        """
        dimensions = {
            "timestamp": [self.log_datetime_field],
    ...
    def anomaly_detection_with_freq_count_per_msgtype(
        self, incident_logrecord, reference_logrecord, stats_per_msgtype
    ):
        """Method to run non-parametric anomaly detetction independently for each message type based on the frequency count statistics

        Args:
            incident_logrecord (LogRecordObject): LogRecord object created from incident period data
            reference_logrecord (LogRecordObject): LogRecord object created from reference period data
            stats_per_msgtype (dict): dict containing anomaly detector stats for each message type.
            This object is taken as input and updated with the anomaly detection results in this method.
            This input is needed to collate the anomaly detection results from multiple reference periods.

        Returns:
            dict : updaed stats_per_msgtype object with anomaly detection results
        """
        incident_attributes = incident_logrecord.attributes
        incident_timestamps = pd.to_datetime(
            incident_logrecord.timestamp[self.log_datetime_field]
        )

        reference_attributes = reference_logrecord.attributes
        reference_timestamps = pd.to_datetime(
            reference_logrecord.timestamp[self.log_datetime_field]
        )

        incident_counts = self.count_feature_extractor.convert_to_counter_vector(
            log_pattern=None,
            attributes=incident_attributes,
            timestamps=incident_timestamps,
        )
        reference_counts = self.count_feature_extractor.convert_to_counter_vector(
            log_pattern=None,
        ...
        incident_counts_msgtype = {
            x[0]: list(x[1][constants.LOG_COUNTS])
            for x in incident_counts.groupby(self.log_msgtype)
        }
        reference_counts_msgtype = {
            x[0]: list(x[1][constants.LOG_COUNTS])
            for x in reference_counts.groupby(self.log_msgtype)
        }
        ...
        all_counts = []
        all_counts.extend(list(incident_counts[constants.LOG_COUNTS]))
        all_counts.extend(list(reference_counts[constants.LOG_COUNTS]))
    ...
    def anomaly_detection_with_metric_per_msgtype(
        self, incident_logrecord, reference_logrecord, stats_per_msgtype
    ):

        """Method to run non-parametric anomaly detetction independently for each message type based on the given metric
        ...
        _, incident_metrics = self.metric_feature_extractor.convert_to_feature_vector(
            None, attributes=incident_attributes, timestamps=incident_timestamps
        )
        _, reference_metrics = self.metric_feature_extractor.convert_to_feature_vector(
            None, attributes=reference_attributes, timestamps=reference_timestamps
        )

        incident_metrics_count = (
            self.metric_feature_extractor.convert_to_counter_vector(
                None, attributes=incident_attributes, timestamps=incident_timestamps
            )
        )
        reference_metrics_count = (
            self.metric_feature_extractor.convert_to_counter_vector(
                None, attributes=reference_attributes, timestamps=reference_timestamps
            )
        )
        ...
        incident_metrics_count_msgtype = {
            x[0]: list(x[1][constants.LOG_COUNTS])
            for x in incident_metrics_count.groupby(self.log_msgtype)
        }
        reference_metrics_count_msgtype = {
            x[0]: list(x[1][constants.LOG_COUNTS])
            for x in reference_metrics_count.groupby(self.log_msgtype)
        }

        incident_metrics_msgtype = {
            x[0]: list(x[1][self.log_metric])
            for x in incident_metrics.groupby(self.log_msgtype)
        }
        reference_metrics_msgtype = {
            x[0]: list(x[1][self.log_metric])
            for x in reference_metrics.groupby(self.log_msgtype)
        }
        ...
        for msgtype in all_msgtypes:
            ...
            reference_metric_data = pd.DataFrame(
                np.repeat(reference_metric_vals, reference_metric_freqs),
                columns=["data"],
            )
            ...
            stats_per_msgtype[msgtype][self.num_lof_field][
                self.log_metric
            ] = num_anomalies
    ...
    def joint_anomaly_detection_with_freq_count_all_msgtypes(
        ...
        """Method to run non-parametric anomaly detetction jointly for each message type based on the given metric
        ...
        incident_counts = incident_counts[[self.log_msgtype, constants.LOG_COUNTS]]
        reference_counts = reference_counts[[self.log_msgtype, constants.LOG_COUNTS]]

        incident_counts_msgtype = incident_counts[self.log_msgtype]
        incident_counts[self.log_msgtype] = encoder.fit_transform(incident_counts)[
            self.log_msgtype + "_categorical"
        ]
        reference_counts[self.log_msgtype] = encoder.fit_transform(reference_counts)[
            self.log_msgtype + "_categorical"
        ]
    ...
    def joint_anomaly_detection_with_metric_all_msgtype(
        self, incident_logrecord, reference_logrecord, stats_per_msgtype
    ):

        """Method to run anomaly anomaly detetction for each message type based on the frequency count statistics
        ...
        incident_metrics_count[self.log_msgtype] = encoder.fit_transform(
            incident_metrics_count
        )[self.log_msgtype + "_categorical"]
        reference_metrics_count[self.log_msgtype] = encoder.fit_transform(
            reference_metrics_count
        )[self.log_msgtype + "_categorical"]
        ...
        incident_metrics_count["isf_anomaly_scores"] = (
            anomaly_scores_isf == -1
        ).astype("float")
        ...
        for msgtype, num_anomalies in incident_isf_scores_per_msgtype.items():
            if msgtype not in stats_per_msgtype:
                stats_per_msgtype[msgtype] = {k: {} for k in self.output_fields}
            stats_per_msgtype[msgtype][self.num_isf_field][
                self.log_metric
            ] = num_anomalies
        return stats_per_msgtype
