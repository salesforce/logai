...
from AIOps.RCA_Log.logai.logai.applications.public_datasets.data_preprocessing.DatasetLogAD import DatasetLogAD, DatasetLogADConfig 
...
class ThunderbirdLogAD(DatasetLogAD):

    def __init__(self, config: DatasetLogADConfig):
        super(ThunderbirdLogAD, self).__init__(config)

    def preprocess_logs(self):
        if not os.path.exists(self.log_file):
            log_data = self.load_data(self.log_file_raw, self.log_format)
            log_data['Time'] = log_data['Date'].astype(str) + "-" + log_data['Time'].astype(str)
...
if __name__=="__main__":
    ...
    config.dataset_config['log_format'] = '<Label> <Id> <Date> <Admin> <Month> <Day> <Time> <AdminAddr> <Content>'
    config.dataset_config['date_time_format'] = '%Y.%m.%d-%H:%M:%S'
    config.dataset_config['custom_replace_list'] = [
                    [r'(/[a-zA-Z0-9\-\_\./]*[\s]?)', '[FILE]'],
                    [r'\d+.\d+.\d+.\d+[#:0-9]*', '[IP]'],
                    [r'(0x)[0-9a-fA-F]+', '[HEX]'],
                    [r'[\-:<>@]*[0-9._]*[a-zA-Z._]*[\-:<>@]*[0-9._]+[\-:<>@]*[0-9._]*[\-:<>@]*', '[ALPHANUM]'],
                    [r'\d+', '[INT]'],
                    [r'Warning: we failed to resolve data source name [\w\s]+', '[WARNING]']
                ]
    config.dataset_config['parsed_date_time_format'] = '%Y-%m-%d %H:%M:%S'
    config.dataset_config['logsequence_delim'] = None
    config.dataset_config['config_file'] = "thunderbird/thunderbird_config_parsefree.yaml"
    config.preprocess_config['train_remove_duplicates'] = True 
    config.preprocess_config['dev_remove_duplicates'] = True 
    tbird_logAD = ThunderbirdLogAD(config=config)
    tbird_logAD.create_dataset()
