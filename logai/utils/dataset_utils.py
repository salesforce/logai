...
def split_train_dev_test_for_anomaly_detection(
    logrecord, training_type, test_data_frac_neg_class, test_data_frac_pos_class=None,  shuffle=False
):
    ...
    train_ids, test_ids, train_labels, test_labels = train_test_split(logrecord.span_id[constants.SPAN_ID],
                                                                      logrecord.labels[constants.LABELS], test_size=0.2,
                                                                      shuffle=shuffle, stratify=stratify)
    if not shuffle:
        stratify = None
    else:
        stratify = train_labels
    train_ids, dev_ids, train_labels, dev_labels = train_test_split(train_ids, train_labels, test_size=0.1,
                                                                    shuffle=shuffle, stratify=stratify)
    ...
    if training_type == "unsupervised":
        ...
        if len(dev_ids) == 0:
            train_ids, dev_ids = train_test_split(train_ids, test_size=0.1, shuffle=shuffle)
    ...
    print("indices_train/dev/test: ", len(indices_train), len(indices_dev), len(indices_test))
