...
from logai.dataloader.parse_nutanix_logs import ParseNutanixLogs
from logai.preprocess.nutanix_logs_masking import MaskLogLine
...
config_path = "../../../examples/jupyter_notebook/nn_ad_benchmarking/configs/nutanix_logbert_config_sm.yaml"
config_parsed = read_file(config_path)
config_dict = config_parsed["workflow_config"]
config = OpenSetADWorkflowConfig.from_dict(config_dict)

dir_path = config.data_loader_config.filepath
logs = []
ITERATOR_SKIP_LIST = ["catalina", "ganesha", "cpp", "samba", "ovsvswitchd", "msp", "messages"]
error_in_parsing = 0
for root, _, files in os.walk(dir_path):
    for filename in files:
        ...
        with open(log_path,'r') as fp:
            logs_list = fp.readlines()
            for log_line in logs_list:
              parser = ParseNutanixLogs()
              iterator, log_details = parser.extract_log_details(log_line)
              if iterator in ITERATOR_SKIP_LIST:
                break
              if "body" not in log_details or "date" not in log_details:
                error_in_parsing += 1
                continue
              logline = log_details["body"]
              masker = MaskLogLine(logline)
              masker.mask_log_line()
              log_details["body"] = masker.log_line
              logs.append(log_details)

logs_df = pd.DataFrame(logs)
logrecord = LogRecordObject.from_dataframe(logs_df)
processed_filepath = os.path.join(config.output_dir, 'nutanix_processed.csv')
...
# TODO change the partition logic
