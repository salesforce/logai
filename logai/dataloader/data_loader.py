...
class FileDataLoader:
    ...
    def _log_to_dataframe(self, fpath, log_format):
        ...
        print("Total size after encoding is", len(log_messages), cnt)
    ...
    def _create_log_record_object(self, df: pd.DataFrame):
        ...
        if not dims:
            selected = pd.DataFrame(df.agg(lambda x: ' '.join(x.values), axis=1).rename(constants.LOGLINE_NAME))
            ...
            for field in LogRecordObject.__dataclass_fields__:
                if field in dims.keys():
                    ...
                    if field == "body":
                        if len(selected.columns) > 1:
                            selected = pd.DataFrame(
                                selected.agg(lambda x: ' '.join(x.values), axis=1).rename(constants.LOGLINE_NAME)
                            )
                        else:
                    ...
                    if field == "timestamp":
                        if len(selected.columns) > 1:
                            selected = pd.DataFrame(
                                selected.agg(lambda x: ' '.join(x.values), axis=1).rename(constants.LOG_TIMESTAMPS)
                            )
