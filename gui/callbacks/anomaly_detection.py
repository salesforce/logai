        ...
        if prop_id == "anomaly-btn":
            try:
                ...
                ad_params = log_anomaly_demo.parse_parameters(
                    ...
                    params={p["Parameter"]: p["Value"] for p in ad_param_table["props"]["data"]
                            if p["Parameter"]},
                )
                parsing_params = LogPattern().parse_parameters(
                    param_info=LogPattern().get_parameter_info(parsing_algo),
                    params={p["Parameter"]: p["Value"] for p in parsing_param_table["props"]["data"]
                            if p["Parameter"]},
                )
...
def update_graph(data, interval):
    ...
    if len(data) > 0:
        ...
        ts_df = df[['timestamp', 'is_anomaly']].groupby(
            pd.Grouper(key='timestamp', freq=freq, offset=0, label='right')) \
...
@callback(
    Output("counter_table", "children"),
    [Input("anomaly-attribute-table", "data")]
)
def update_counter_table(data):
    # print(attributes)
    if len(data) > 0:
        df = log_anomaly_demo.get_counter(data[0])[['timestamp', constants.LOGLINE_COUNTS]]
...
def update_anomaly_table(data):
    if len(data) > 0:
        df = log_anomaly_demo.get_anomalies(data[0])[[constants.LOG_TIMESTAMPS, constants.LOGLINE_NAME]]
        columns = [{"name": c, "id": c} for c in df.columns]
        return dash_table.DataTable(
            data=df.to_dict("records"),
            columns=columns,
...
@callback(
    Output("ad-param-table", "children"),
    Input("ad-algo-select", "value")
)
...
@callback(
    Output("ad-parsing-param-table", "children"),
