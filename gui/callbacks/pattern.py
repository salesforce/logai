...
from logai.applications.application_interfaces import WorkFlowConfig, \
    FeatureExtractorConfig, PreprocessorConfig, \
    LogParserConfig
...
def create_attribute_component(attributes):
    table = dash_table.DataTable(
        ...
        columns=[{'id': c, 'name': c, 'presentation': 'dropdown'} for c in attributes.columns],
        editable=True,
        dropdown={
            c: {
                'options': [{'label': '*', 'value': '*'}] + [{'label': i, 'value': i} for i in attributes[c].unique()]
            }
        ...
        if ctx.triggered:
            ...
            if prop_id == "pattern-btn":
                ...
                params = log_pattern_demo.parse_parameters(
                    ...
                    params={p["Parameter"]: p["Value"] for p in param_table["props"]["data"] if p["Parameter"]},
                )

                config = _config_sample()
                config.open_set_data_loader_config.filepath = file_path  # overwrite the file path.
...
def update_dynamic_lists(data):
    if data is not None:
        df = log_pattern_demo.get_dynamic_parameter_list(data['points'][0]['customdata'])
        df["values"] = df["values"].apply(lambda x: ", ".join(set(filter(None, x))))
        df = df.rename(columns={"position": "Position", "value_counts": "Count", "values": "Value"})
...
def update_summary_graph(data):
    ...
    fig = px.bar(scatter_df,
                 x='order',
                 y='counts',
                 labels={
                     'order': 'log pattern',
                     'counts': 'Occurrence (Log Scale)'
                 },
...
def update_y_timeseries(data, interval):
    ...
    ts_df = dff[['timestamp', 'parsed_logline']].groupby(
        pd.Grouper(key='timestamp', freq=freq, offset=0, label='right')).size().reset_index(name='count')
...
def create_time_series(dff, axis_type, title):
    fig = px.scatter(dff, x='timestamp', y='count',
                     labels={
                         'count': 'Occurrence',
                         'timstamp': 'Time'
                     },
...
@callback(
    Output("parsing-param-table", "children"),
