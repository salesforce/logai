        ...
        if prop_id == "clustering-btn":
            try:
                ...
                clustering_params = log_clustering.parse_parameters(
                    ...
                    params={p["Parameter"]: p["Value"] for p in clustering_param_table["props"]["data"]
                            if p["Parameter"]},
                )
                ...
                return create_attribute_component(log_clustering.get_attributes()), \
                       False, ""
...
@callback(
    Output("clustering-loglines", "children"),
    [Input("cluster-hist", "clickData")]
)
